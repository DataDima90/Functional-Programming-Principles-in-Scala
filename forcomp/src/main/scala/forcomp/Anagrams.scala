...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.toLowerCase.toSeq.groupBy(identity).map(element => (element._1, element._2.length)).toList.sorted
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    x.map(occX => y.find(_._1 == occX._1) match {
      case Some(occY) => (occX._1, occX._2 - occY._2)
      case None => occX
    }).filter(_._2 != 0)
  }
